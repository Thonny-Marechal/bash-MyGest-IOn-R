...
Ce programme permet d'aider au quotidien les techniciens réseaux. En effet il permet de vérifier et consulter plusieurs informations essentielles au bon fonctionnement d'une configuration réseau. L'objectif est donc de pouvoir gagner du temps pour la gestion de son parc informatique grace à des scripts en Bash. Mais on peut également gérer une base de données SQL
...
Cette mission avait pour but de pouvoir offrir des éléments qui seront utiles pour les techniciens réseaux. Ce sont donc des petits scripts en Bash qui ont été concus afin de tester une IP, un port TCP, générer un .txt depuis la base de données et pour finir consulter les IP actives ou non de la base de donnçes.
Partie 3 : Installateur

Ici c'est l'installation des paquets au bon fonctionnement des scripts. De ce fait, au lancement du MyGestIOnR.sh, il y aura une installation automatique des paquets.
